...
- **2026-03-29** — `oxfmt / package.json`: oxfmt reorders `package.json` fields (keywords, license, author, repository, files, dependencies) as a formatting side-effect. This is expected behavior, not scope creep. Reviewers should not flag these reorderings as violations. (from task authkit-session-mncbvowg-ideation-oxfmt-oxlint-migratio)
