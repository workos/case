...
1. A human fills in a task template (from `tasks/templates/`) and creates a `td` issue in the target repo's `.todos/` store.
...
6. After merge, the task record's status is updated in the repo's `td` store.
