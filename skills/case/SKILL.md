...
- [ ] **Screenshots/video of the SPECIFIC fix.** Capture evidence that your change works — not a generic "the app loads" screenshot. For a bug fix: show the behavior BEFORE (on main, with the bug) and AFTER (on your branch, fixed). For a feature: show the feature working. If you can't demonstrate the fix visually, explain why in the PR description. Skip ONLY for pure backend/CLI/types-only changes.
